...
@objc(AmarAvroKeyboardInputController)
class AmarAvroKeyboardInputController: IMKInputController {
    ...
    static let typingModeKey = "AmarAvroKeyboardTypingMode"

    /// Legacy bool key (pre-multi-mode). Read only for one-time migration into
    /// `typingModeKey`: true → `.phoneticOnly`, false → `.smart`.
    static let phoneticOnlyModeKey = "AmarAvroKeyboardPhoneticOnlyMode"
    ...
    override init!(server: IMKServer!, delegate: Any!, client inputClient: Any!) {
        ...
        NotificationCenter.default.addObserver(
            ...
            name: .amarAvroKeyboardTypingModeChanged,
            object: nil
        )
    }
    ...
    private func getUserDataDir() -> String {
        let appSupport = FileManager.default.urls(
            ...
            in: .userDomainMask
        ).first!.appendingPathComponent("Amar Avro Keyboard")
    ...
    override func handle(_ event: NSEvent!, client sender: Any!) -> Bool {
        ...
        if !riti_context_ongoing_input_session(engineCtx),
           ...
           digit >= "0" && digit <= "9" {
            ...
            let bengaliDigit = String(AmarAvroKeyboardInputController.bengaliDigits[digitValue])
            client.insertText(
...
extension Notification.Name {
    static let amarAvroKeyboardTypingModeChanged = Notification.Name("AmarAvroKeyboardTypingModeChanged")
}
