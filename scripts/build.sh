...
SWIFT_DIR="$PROJECT_ROOT/AmarAvroKeyboard"
...
BUILD_UNIVERSAL="${2:-true}"

echo "=== Amar Avro Keyboard Build ==="
...
# Build Apple Silicon. A universal build also compiles the Intel slice.
...
# Copy the official brand icon for the app and macOS input menu.
...
if [ "$BUILD_UNIVERSAL" = "true" ]; then
    ...
    swiftc "${SWIFT_SOURCES[@]}" "${SWIFT_FLAGS[@]}" \
        ...
        -target "arm64-apple-macosx$DEPLOYMENT_TARGET" \
        -o "$APP_BUNDLE/Contents/MacOS/${EXECUTABLE_NAME}_arm64"
    ...
    swiftc "${SWIFT_SOURCES[@]}" \
        ...
        -o "$APP_BUNDLE/Contents/MacOS/${EXECUTABLE_NAME}_x86_64"

    echo ">>> Creating universal Swift binary..."
    lipo -create \
        "$APP_BUNDLE/Contents/MacOS/${EXECUTABLE_NAME}_arm64" \
        "$APP_BUNDLE/Contents/MacOS/${EXECUTABLE_NAME}_x86_64" \
        -output "$APP_BUNDLE/Contents/MacOS/$EXECUTABLE_NAME"

    rm "$APP_BUNDLE/Contents/MacOS/${EXECUTABLE_NAME}_arm64"
    rm "$APP_BUNDLE/Contents/MacOS/${EXECUTABLE_NAME}_x86_64"
else
    swiftc "${SWIFT_SOURCES[@]}" "${SWIFT_FLAGS[@]}"
fi
...
codesign --force --sign - \
    --entitlements "$SWIFT_DIR/Resources/AmarAvroKeyboard.entitlements" \
    "$APP_BUNDLE"
